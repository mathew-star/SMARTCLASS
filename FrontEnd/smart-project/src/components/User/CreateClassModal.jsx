...
function CreateClassModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ title, sections, description });
    onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      <DialogContent className="sm:max-w-[425px] bg-[#151A2C] text-white p-8 rounded-lg shadow-lg w-96">
        <DialogHeader>
          <DialogTitle className="text-2xl mb-6">Create Classroom</DialogTitle>
          <DialogDescription>
        ...
        <div className="mt-2">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              <label className="block text-gray-200 text-sm font-bold mb-2" htmlFor="sections">
                Section
              </label>
              <input
                id="sections"
